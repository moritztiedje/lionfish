...
class View(metaclass=ABCMeta):
    ...
    def display(self, game_state):
        ...
        """
        for button in self.__buttons:
            button.display(self.__game_window)

    def _display_cell(self, image, coordinate):
        """
        ...
        self.__game_window.blit(image, display_coordinate)
    ...
    def display(self, game_state):
        ...
        for x in range(0, len(area_map)):
            for y in range(0, len(area_map[x])):
                if area_map[x][y] == 1:
                    self._display_cell(self.__border, Point(x, y))
                if area_map[x][y] == 0:
                    self._display_cell(self.__water, Point(x, y))
